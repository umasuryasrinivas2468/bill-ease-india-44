  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .or(`user_id.eq.${normalizedUserId},is_default.eq.true`)
        .eq('is_active', true)
        .order('category_name');
...
export const useExpensesByProject = (projectId: string | undefined) => {
  ...
  return useQuery({
    queryKey: ['expenses-by-project', user?.id, projectId],
    queryFn: async () => {
      if (!user || !isValidUserId(user.id) || !projectId) return [];
      const normalizedUserId = normalizeUserId(user.id);
      const { data, error } = await supabase
        .from('expenses')
        .select('*')
    ...
    enabled: !!user && isValidUserId(user?.id) && !!projectId,
  });
};
