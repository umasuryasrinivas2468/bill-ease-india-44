...
app.post('/send-license-email', async (req, res) => {
  try {
    const { email, licenseKey, planType, expiryDate }: LicenseEmailData = req.body;
...
const generateWelcomeLetter = (): string => {
  return `
...
const getEmailTemplate = (data: LicenseEmailData): string => {
  return `
<!DOCTYPE html>
<html>
<head>
...
const generateLicensePDF = async (data: LicenseEmailData): Promise<Buffer> => {
  const doc = new jsPDF();
