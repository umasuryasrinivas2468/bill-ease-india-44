...
import {
  ...
  useSalesReturnWithItems,
} from '@/hooks/useSalesReturns';
import { useInvoices } from '@/hooks/useInvoices';
import { Plus, Search, RotateCcw, CheckCircle2, XCircle, Eye, FileText } from 'lucide-react';
...
const SalesReturns: React.FC = () => {
  ...
  const { data: detail } = useSalesReturnWithItems(detailReturnId);

  const filteredInvoices = useMemo(
    () => (invoices as Invoice[]).filter((i) => i.status !== 'paid' || true),
    [invoices],
  );
  const pickedInvoice: Invoice | null =
    pickedInvoiceId ? (invoices as Invoice[]).find((i) => i.id === pickedInvoiceId) ?? null : null;
  ...
  const summary = useMemo(() => {
    ...
    const totalCredit = approved.reduce((s, r) => s + Number(r.total_amount || 0), 0);
    return {
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast({ title: 'Return approved', description: 'Credit note issued and journals posted.' });
    } catch (e: any) {
    ...
    <div className="p-4 md:p-6 space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">Sales Returns</h1>
            <p className="text-muted-foreground">
              Track product returns, auto-issue credit notes, restore inventory & reverse GST.
            </p>
          </div>
        </div>
        <Button variant="orange" onClick={openCreateForInvoice}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><CardContent className="p-4">
          <div className="text-sm text-muted-foreground">Total Credit Issued</div>
          <div className="text-2xl font-semibold">₹{summary.totalCredit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
        </CardContent></Card>
      </div>
            ...
            <div className="p-8 text-center">
              ...
              <div className="text-muted-foreground mb-4">
                {search || statusFilter !== 'all'
                  ...
                  : 'No sales returns yet. Create one from an invoice.'}
              </div>
              {!search && statusFilter === 'all' && (
                <Button variant="orange" onClick={openCreateForInvoice}>
      ...
      <Dialog open={pickInvoiceOpen} onOpenChange={setPickInvoiceOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Select Invoice to Return</DialogTitle>
          </DialogHeader>
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((inv) => (
                    <SelectItem key={inv.id} value={inv.id}>
                      {inv.invoice_number} — {inv.client_name} — ₹{Number(inv.total_amount).toLocaleString()}
                    </SelectItem>
                  ))
                )}
              </SelectContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPickInvoiceOpen(false)}>Cancel</Button>
            <Button variant="orange" onClick={proceedToCreate} disabled={!pickedInvoiceId}>
              Continue
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          {detail?.header && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div><div className="text-muted-foreground">Invoice</div><div className="font-medium">{detail.header.invoice_number}</div></div>
                <div><div className="text-muted-foreground">Customer</div><div className="font-medium">{detail.header.customer_name}</div></div>
                ...
                <div><div className="text-muted-foreground">COGS Reversed</div><div>₹{Number(detail.header.cogs_reversed || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div></div>
                <div><div className="text-muted-foreground">Credit Note</div>
                  <div className="font-medium">{detail.header.credit_note_id ? <span className="inline-flex items-center gap-1"><FileText className="h-3 w-3" /> Issued</span> : '—'}</div>
                </div>
              </div>
                  ...
                  <TableBody>
                    {(detail.items || []).map((it: any) => (
                      <TableRow key={it.id}>
                        ...
                        <TableCell className="capitalize">{it.condition}</TableCell>
