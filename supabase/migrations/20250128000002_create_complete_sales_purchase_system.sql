-- Create complete Sales Orders, Purchase Orders, Receivables, and Payables system
...
DROP POLICY IF EXISTS "Users can only see their own vendors" ON vendors;
DROP POLICY IF EXISTS "Users can only see their own sales_orders" ON sales_orders;
DROP POLICY IF EXISTS "Users can only see their own purchase_orders" ON purchase_orders;
DROP POLICY IF EXISTS "Users can only see their own receivables" ON receivables;
DROP POLICY IF EXISTS "Users can only see their own payables" ON payables;
...
DROP FUNCTION IF EXISTS create_receivable_from_sales_order();
DROP FUNCTION IF EXISTS create_payable_from_purchase_order();
...
BEGIN
    ...
    IF NEW.status = 'confirmed' AND NEW.payment_status != 'paid' THEN
        -- Check if receivable already exists for this sales order
        IF NOT EXISTS (SELECT 1 FROM receivables WHERE related_sales_order_id = NEW.id) THEN
...
BEGIN
    ...
    IF NEW.status = 'confirmed' AND NEW.payment_status != 'paid' THEN
        -- Check if payable already exists for this purchase order
...
-- Drop existing business logic triggers if they exist
DROP TRIGGER IF EXISTS create_receivable_trigger ON sales_orders;
DROP TRIGGER IF EXISTS create_payable_trigger ON purchase_orders;
